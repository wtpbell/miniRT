...
/*   Updated: 2025/05/31 19:11:17 by bewong           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_col32	apply_ambient(t_col32 base_col, t_light *light)
{
	return (v3f_to_col32((t_v3f){
		.x = col32_to_v3f(base_col).x * col32_to_v3f(light->col).x * light->intensity,
		.y = col32_to_v3f(base_col).y * col32_to_v3f(light->col).y * light->intensity,
		.z = col32_to_v3f(base_col).z * col32_to_v3f(light->col).z * light->intensity
	}));
}

void	init_lighting(t_lighting *lighting, t_ray_hit *hit, t_light *light, t_v3f camera_pos)
{
...
float	calculate_specular(t_lighting *lighting, float shininess, float specular_strength)
{
	...
	refl_dir = v3f_refl(v3f_scale(lighting->light_dir, -1.0f), lighting->normal);
	spec = powf(ft_maxf(0.0f, v3f_dot(lighting->view_dir, refl_dir)), shininess);
...
t_col32	apply_point(t_scene *scene, t_ray_hit *hit, t_light *light)
{
	...
	intensity = lt.intensity / (1.0f + 0.1f * lt.distance + 0.01f * lt.distance * lt.distance);
	...
	if (find_intersection(&ray, scene, &shadow_dist) && shadow_dist < lt.distance - BIAS)
	...
	return (v3f_to_col32((t_v3f){
		.x = (colors[0].x * colors[1].x * lt.diffuse + colors[1].x * lt.specular) * intensity,
		.y = (colors[0].y * colors[1].y * lt.diffuse + colors[1].y * lt.specular) * intensity,
		.z = (colors[0].z * colors[1].z * lt.diffuse + colors[1].z * lt.specular) * intensity
	}));
}
