...
/*   Updated: 2025/08/04 18:18:45 by jboon         ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
	...
	t_result *res)
{
	...
	if (v < 0.0f || (u + v) > 1.0f)
		return (0);
	res->t = v3f_dot(vars->v0v2, qvec) * vars->det;
	res->tri_weight.u = u;
	res->tri_weight.v = v;
	return (1);
}
...
int	tri_intersect(t_tri *tri, t_ray *ray, t_v2f t, t_result *res)
{
	...
	if (fabs(var.det) > FLT_SML && is_within_triangle(tri, ray, &var, res))
		return (res->t > t.x && res->t < t.y);
	return (0);
}
...
{
	return (tri_intersect(&obj->tri, ray, t, res));
}
